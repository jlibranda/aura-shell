...
/** A Prisma client able to own an interactive transaction spanning legal entity, audit, and outbox tables. */
export type PrismaLegalEntityTransactionRunner = Readonly<{
  $transaction<T>(fn: (client: Prisma.TransactionClient) => Promise<T>): Promise<T>;
}>;

/**
 * Durable transaction coordinator for legal entities. Mirrors
 * PrismaLocationUnitOfWork/PrismaOrgUnitUnitOfWork: legal entity writes,
 * audit records, and outbox messages commit atomically inside one Prisma
 * $transaction; domain events are released to their collector only after
 * that commit succeeds.
 */
export class PrismaLegalEntityUnitOfWork implements UnitOfWork<LegalEntityTransactionRepositories> {
  constructor(
    private readonly prisma: PrismaLegalEntityTransactionRunner,
  ...
  async execute<TResult>(
    ...
    const committed = await this.prisma.$transaction(async (client) => {
      const legalEntities = new LegalEntityWriteTransaction(new PrismaLegalEntityWriteRepository(client), transactionContext);
  ...
  private createAuditRecords(context: UnitOfWorkTransaction<LegalEntityTransactionRepositories>["context"], events: readonly DomainEvent[]) {

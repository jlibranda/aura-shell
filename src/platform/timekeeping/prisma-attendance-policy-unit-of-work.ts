...
import { PrismaAttendancePolicyWriteRepository } from "@/platform/timekeeping/prisma-attendance-policy-write-repository";
...
/** A Prisma client able to own an interactive transaction spanning attendance policy, audit, and outbox tables. */
export type PrismaAttendancePolicyTransactionRunner = Readonly<{
  $transaction<T>(fn: (client: Prisma.TransactionClient) => Promise<T>): Promise<T>;
}>;

/**
 * Durable transaction coordinator for AttendancePolicy. Mirrors
 * PrismaScheduleAssignmentUnitOfWork: the write (create/end), audit
 * records, and outbox messages commit atomically inside one Prisma
 * $transaction; domain events are released to their collector only after
 * that commit succeeds.
 */
export class PrismaAttendancePolicyUnitOfWork implements UnitOfWork<AttendancePolicyTransactionRepositories> {
  constructor(
    private readonly prisma: PrismaAttendancePolicyTransactionRunner,
  ...
  async execute<TResult>(
    ...
    const committed = await this.prisma.$transaction(async (client) => {
      const attendancePolicies = new AttendancePolicyWriteTransaction(new PrismaAttendancePolicyWriteRepository(client), transactionContext);
  ...
  private createAuditRecords(context: UnitOfWorkTransaction<AttendancePolicyTransactionRepositories>["context"], events: readonly DomainEvent[]) {
